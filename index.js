...
!(function (bindObject, factory) {
  ...
  function success(type, time, res) {
    return (
      "n/a" !== type &&
        (!(function (a, c, partKeys, paramName) {
          /** @type {string} */
          var k = a + "=" + c;
          /** @type {!Date} */
          var date = new Date();
        ...
        })(res || guid(), getDate(), 365, time),
        removeItem(guid(), getDate())),
      type
    );
  }
  /**
  ...
  var add = function (e, v) {
    ...
    return (
      ...
      (n[0] += n[1] >>> 16),
      (n[1] &= 65535),
      (n[0] += e[0] * v[3] + e[1] * v[2] + e[2] * v[1] + e[3] * v[0]),
      (n[0] &= 65535),
      [(n[0] << 16) | n[1], (n[2] << 16) | n[3]]
    );
  };
  ...
  var table = function (value) {
    return (
      (value = get(value, [0, value[0] >>> 1])),
      (value = add(value, [4283543511, 3981806797])),
      (value = get(value, [0, value[0] >>> 1])),
      (value = add(value, [3301882366, 444984403])),
      (value = get(value, [0, value[0] >>> 1]))
    );
  };
  /**
  ...
  var load = function () {
    return (function (text, a) {
      ...
      for (; i < removeCount; i = i + 16) {
        /** @type {!Array} */
        e = [
          (255 & text.charCodeAt(i + 4)) |
            ((255 & text.charCodeAt(i + 5)) << 8) |
            ((255 & text.charCodeAt(i + 6)) << 16) |
            ((255 & text.charCodeAt(i + 7)) << 24),
          (255 & text.charCodeAt(i)) |
            ((255 & text.charCodeAt(i + 1)) << 8) |
            ((255 & text.charCodeAt(i + 2)) << 16) |
            ((255 & text.charCodeAt(i + 3)) << 24),
        ];
        /** @type {!Array} */
        c = [
          (255 & text.charCodeAt(i + 12)) |
            ((255 & text.charCodeAt(i + 13)) << 8) |
            ((255 & text.charCodeAt(i + 14)) << 16) |
            ((255 & text.charCodeAt(i + 15)) << 24),
          (255 & text.charCodeAt(i + 8)) |
            ((255 & text.charCodeAt(i + 9)) << 8) |
            ((255 & text.charCodeAt(i + 10)) << 16) |
            ((255 & text.charCodeAt(i + 11)) << 24),
        ];
        e = add(e, g);
        e = walk(e, 31);
        e = add(e, v);
      ...
      return (
        (data = get(data, [0, text.length])),
        (value = get(value, [0, text.length])),
        (data = normalize(data, value)),
        (value = normalize(value, data)),
        (data = table(data)),
        (value = table(value)),
        (data = normalize(data, value)),
        (value = normalize(value, data)),
        ("00000000" + (data[0] >>> 0).toString(16)).slice(-8) +
          ("00000000" + (data[1] >>> 0).toString(16)).slice(-8) +
  ...
  var init = function () {
    try {
      return definition()
        ? new Promise(function (saveNotifs) {
            if (iOSInfo() >= 76) {
    ...
    k21: function (wfcb) {
      !(function (cb) {
        ...
        var autoResumeTimer = setTimeout(function () {
          return (context.oncomplete = function () {}), cb(-3);
        }, 1e3);
        /**
         * @param {?} event
         * @return {?}
